...
- Declared (all required by `@capacitor/camera` 7): **`NSPhotoLibraryUsageDescription`**
  (import), **`NSCameraUsageDescription`** (Take Photo), and
  **`NSPhotoLibraryAddUsageDescription`** (photo-library access for images added to the
  canvas).
